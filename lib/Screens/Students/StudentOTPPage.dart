...
// bool loading = true;
    ...
    if (AllData.length == 0) {
      ...
      setState(() {
        ...
        for (var i = 0; i < AllData.length; i++) {
          var otpCode = AllData[i]["StudentPhoneOtpCode"];
          ...
          if (otpCode == "${StudentOTPController.text.trim()}") {
            ...
            final UpadateData = {"StudentPhoneVerify": "true"};
  ...
  Future ResendOTPSend() async {
    ...
    final response = await http.get(Uri.parse(
        'https://api.greenweb.com.bd/api.php?token=1024519252916991043295858a1b3ac3cb09ae52385b1489dff95&to=${widget.StudentPhoneNumber}&message=${OtpMsg}'));

    if (response.statusCode == 200) {
      final docUser = FirebaseFirestore.instance
          .collection("StudentInfo")
          .doc(widget.StudentPhoneNumber);

      final UpadateData = {"StudentPhoneOtpCode": code.toString()};
                          ...
                          children: [
                            Container(
                                ...
                                child: Padding(
                                  padding: const EdgeInsets.all(8.0),
                                  child: Text(
                                    "আপনার ${widget.StudentPhoneNumber} এ OTP পাঠানো হয়েছে",
